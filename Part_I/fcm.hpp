...
class fcm 
{
    ...
    public:
         ...
         * @param filename name of text file to be analyzed
         * @param toSave flag that indicates if the model should be saved in memory (Y/N)
         */
        void estimateFileEntropy(char *filename, char toSave);
         ...
         * @param filename name of text file from which the model was generated
         */
        void saveModel(map<string, map<char, int>> model, char *filename);
        ...
        */ 
        ...
        {
            // check if string is valid float/double
            if(s.empty() || ((!isdigit(s[0])) && (s[0] != '-') && (s[0] != '+')))
            {
                return false;
            } 
